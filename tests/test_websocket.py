...
def test_websocket_observeproperty_counter():
    with TestClient(server.app) as client:
        with client.websocket_connect("/my_thing/ws") as ws:
            ...
            message = ws.receive_json(mode="text")
            assert message["data"]["counter"] == 1  # Expect the counter to be 1 after increment
            ws.close(1000)
            my_thing.counter = 0 # Set counter back to 0


def handle_websocket_messages(message):
    if message["messageType"] == 'actionStatus' and message["data"]["status"] == 'completed':
        return True
    return False
...
def test_websocket_observeaction():
    with TestClient(server.app) as client:
        with client.websocket_connect("/my_thing/ws") as ws:
            ...
            ws.send_json(
                {"messageType": "addActionObservation", "data": {"slowly_increase_counter": True}}
            )

            # Trigger the slowly_increase_counter action
            client.post("/my_thing/slowly_increase_counter")
