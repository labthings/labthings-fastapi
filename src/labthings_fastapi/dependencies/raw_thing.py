...
def find_raw_thing_by_class(
    ...
    **WARNING**: using a raw `Thing` can be tricky: unless you really need to, it is
    ...
    def find_raw_thing(
        ...
        This function is generated by `find_raw_thing_by_class`, see that for docs.
        """
...
def raw_thing_dependency(cls: type[ThingInstance]) -> type[ThingInstance]:
    """Generate an annotated type that will supply a Thing at runtime.
    ...
    **WARNING**: using a raw `Thing` can be tricky: unless you really need to, it is
    ...
    This function should make it simple to depend on a `Thing` object directly. If
    ...
    return Annotated[
        cls,
        Depends(find_raw_thing_by_class(cls))
