...
`labthings-fastapi` automatically generates a `Thing Description`_ to describe each `~lt.Thing`. Each function offered by the `~lt.Thing` is either a Property, Action, or Event. These are termed "interaction affordances" in WoT_ terminology.
