...
Collecting metadata
--------------------
It's relatively common to want to collect metadata that summarises all of the `~lt.Thing`
...
`.ThingServerInterface.get_thing_states` will return a mapping of `~lt.Thing` names to
metadata dictionaries, suitable for embedding in a data file. The metadata dictionaries
are generated by the ``thing_state`` property of each `~lt.Thing`\ .
