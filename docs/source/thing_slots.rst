...
LabThings-FastAPI provides `~lt.thing_slot`\ . This allows a `~lt.Thing`
...
worry about the order in which `~lt.Thing`\ s are created.

The following example shows the use of a `~lt.thing_slot`:
...
is type hinted as a `~lt.Thing` subclass, and by default the server will look for the
instance of that class and supply it when the server starts. If there is no
matching `~lt.Thing` or if more than one instance is present, the server will fail
to start with a `.ThingSlotError`\ .

It is also possible to use an optional type hint (``ThingA | None``), which
means there will be no error if a matching `~lt.Thing` instance is not found, and
the slot will evaluate to `None`\ . Finally, a `~lt.thing_slot` may be
...
A `~lt.thing_slot` may be given a default value. If this is a string, the server
will look up the `~lt.Thing` by name. If the default is `None` the slot will
evaluate to `None` unless explicitly configured.

Slots may also be specified in the server's configuration:
`~lt.ThingConfig` takes an argument that allows connections to be made
